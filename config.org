...
** Environment tweaks
This is to set the bash environment variables so the emacs binary is found when
tangling this file.
...
#+begin_src emacs-lisp
(setq user-full-name "Himanshu Singh"
      user-mail-address "to.himanshu.singh@gmail.com")
#+end_src
...
(display-battery-mode 1)                          ; Always display as the conditional was erroring out in doom doctor
...
;; (julia +lsp)                 ; Python, R, and MATLAB in a blender (Disabled as no M1 support)
...
*** Visual Settings
**** Font Face
'Fira Code' ('Fira Code Retina') is nice, and 'Overpass' ('Alegreya') makes for a nice sans companion. We just need to
    ...
    (org-element-cache-map
     (lambda (src)
       (when (and (not (org-in-commented-heading-p nil src))
                  ...
                  (equal (alist-get :noweb-ref (org-babel-parse-header-arguments
                                                (org-element-property :parameters src) t))
                         name))
         (push (string-trim-right (org-element-property :value src) "\n")
               block-contents)))
     :granularity 'element
...
cd /tmp
curl -o "hunspell-en-custom.zip" 'http://app.aspell.net/create?max_size=80&spelling=US&max_variant=0&diacritic=keep&special=hacker&special=roman-numerals&encoding=utf-8&format=inline&download=hunspell'
...
cd /tmp
curl -o "aspell6-en-custom.tar.bz2" 'http://app.aspell.net/create?max_size=80&spelling=US&spelling=AU&max_variant=0&diacritic=keep&special=hacker&special=roman-numerals&encoding=utf-8&format=inline&download=aspell'
...
For when you want to export to Jira
#+begin_src emacs-lisp :tangle packages.el
(package! ox-jira)
#+end_src

*** Poetry mode
1. This needs =rhyme= installed from https://github.com/shaunplee/homebrew-rhyme
2. Then =poetry.el= from http://www.bobnewell.net/filez/poetry.el
...
#+begin_src emacs-lisp :tangle packages.el
(package! notes-migrator :recipe (:host github :repo "bitspook/notes-migrator"))
#+end_src
...
Get the GCP client ID & the secret from environment variables

#+begin_src emacs-lisp
(require 'org-gcal)
(setq org-gcal-client-id (getenv "ORG_GCAL_CLIENT_ID")
      org-gcal-client-secret (getenv "ORG_GCAL_CLIENT_SECRET")
      org-gcal-fetch-file-alist `((,(print (getenv "ORG_GCAL_CAL_ID_1")) . "~/orgfiles/org/nextcloud/Himanshu/cal-gmail.org")
                                  (,(print (getenv "ORG_GCAL_CAL_ID_2")) . "~/orgfiles/org/nextcloud/Family/cal-family.org")))
#+end_src
...
(use-package! org-pandoc-import
  :after org)
  ;; Google Drive integration https://www.emacswiki.org/emacs/GoogleDocs
;; https://github.com/prasmussen/gdrive
(defvar gdocs-folder-id "1MGWas-zzpm8ktv2y32gQQRH5FcK9Gyi1"
 ...
 (defun gdoc-export-buffer ()
      ...
      (interactive)
      (shell-command
      (format "gdrive upload --mime text/plain --parent %s --file %s"
          gdocs-folder-id buffer-file-name)))
;;  gdrive import --parent '1MGWas-zzpm8ktv2y32gQQRH5FcK9Gyi1' 2021-06-16.txt
(defun gdoc-import-buffer (doc)
       ...
       ;;  gdrive list -q "'1MGWas-zzpm8ktv2y32gQQRH5FcK9Gyi1' in parents"
...
;; gdrive export --mime text/plain  1q-cDyKWyM1uzXrjwf0DJbN4LqPesMZo9wURdwkvcFZg
#+end_src
...
#+begin_src emacs-lisp
(setq org-directory "~/orgfiles/org"       ; let's put files here
...
(defvar org-babel-lang-list
  '("go" "python" "ipython" "bash" "sh" "swift"))
...
(after! org-capture
  ...
  (defvar +org-capture-recipies  "~/orgfiles/org/nextcloud/Family/recipies.org")

  (setq +org-capture-todo-file "~/orgfiles/org/nextcloud/Himanshu/todo.org")

  (setq org-default-notes-file "~/orgfiles/org/nextcloud/Himanshu/notes.org")
...
Unpin Org Roam to checkout the latest release
#+begin_src emacs-lisp :noweb-ref none :tangle packages.el
(unpin! org-roam) ;pull in changes for import
#+end_src
...
#+begin_src emacs-lisp :noweb-ref none :tangle (if (file-exists-p "~/orgfiles/org/roam/") "no" "packages.el")
...
#+end_src
***** Capture & Agenda customizations
From David Wilson's config https://systemcrafters.net/build-a-second-brain-in-emacs/5-org-roam-hacks/
...
#+end_src
***** Fix ID synchronization with org
Fix for org-id not found https://org-roam.discourse.group/t/org-roam-v2-org-id-id-link-resolution-problem/1491/4
#+begin_src emacs-lisp :noweb-ref none :tangle yes
...
** Plantuml
Set the location of the plantuml jar file
#+begin_src emacs-lisp
