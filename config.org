...
cd /tmp
curl -o "hunspell-en-custom.zip" 'http://app.aspell.net/create?max_size=80&spelling=USs&max_variant=0&diacritic=keep&special=hacker&special=roman-numerals&encoding=utf-8&format=inline&download=hunspell'
...
***** Fix Doom breakage
This is to fix breaking changes in doom =Could not find package gitconfig-mode= which can be fixed by the following workaround
[[https://github.com/hlissner/doom-emacs/issues/5667#issuecomment-948229579][https://github.com/hlissner/doom-emacs/issues/5667#issuecomment-948229579]]
